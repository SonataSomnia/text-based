tyrano.plugin.kag.parser = {
  ...
  parseScenario: function (text_str) {
    for (
      ...
      if (!0 === flag_comment && "*/" === line_str) flag_comment = !1;
        ...
        const tag_str = line_str.substr(1, line_str.length),
        ...
        for (var j = 0; j < array_char.length; j++) {
          var c = array_char[j];
          if (this.flag_script) text += c;
          else if (scanning_tag)
            if ("]" === c)
              if ("" !== start_quot) tag_str += c;
              else {
                deep_kakko--;
                if (0 === deep_kakko) {
                  scanning_tag = !1;
              ...
              "" === start_quot && deep_kakko++;
              tag_str += c;
            } else if ('"' === c || "'" === c || "`" === c) {
              start_quot === c
                ? (start_quot = "")
                : "" === start_quot && (start_quot = c);
              tag_str += c;
            } else tag_str += c;
          else if (flag_escape) {
            text += c;
            ...
            if ("" != text) {
              const text_obj = {
                ...
                pm: { val: text },
                val: text,
                is_entity_disabled: !0,
              };
              array_s.push(text_obj);
              text = "";
            }
          } else "\\" === c ? (flag_escape = !0) : (text += c);
        }
        if (tag_str) {
          if ("]" === tag_str.slice(-1)) {
            ...
            this.kag.warning(
              ...
              { before: tag_str, after: tag_str_fixed },
              !1
            );
  ...
  makeTag: function (str, line) {
    var obj = { line: line, name: "", pm: {}, val: "" },
      ...
      keepSpaceConfig = this.kag.config.KeepSpaceInParameterValue;
    function makeParam() {
      obj.pm[param_name] = param_value;
      var param_value_trim = $.trim(param_value);
      "undefined" === param_value_trim && (obj.pm[param_name] = "");
      "3" !== keepSpaceConfig && (obj.pm[param_name] = param_value_trim);
    }
    for (var j = 0; j < array_c.length; j++) {
      var c = array_c[j];
      switch (scanning_state) {
        case 1:
          " " === c ? "" === tag_name || (scanning_state = 2) : (tag_name += c);
          break;
        case 2:
          " " === c
            ? "" === param_name || (scanning_state = 3)
            : "=" === c
            ? (scanning_state = 4)
            : (param_name += c);
          break;
        case 3:
          if ("=" === c) scanning_state = 4;
          else if (" " === c);
          else {
            obj.pm[param_name] = "";
            param_name = c;
