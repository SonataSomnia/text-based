tyrano.plugin.kag.ftag = {
  ...
  init: function () {
    for (var order_type in tyrano.plugin.kag.tag) {
      this.master_tag[order_type] = object(tyrano.plugin.kag.tag[order_type]);//Note:此处涉及到数组法和属性法的转换
      ...
      if (this.master_tag[tag.name]) {
        ...
        if ("" != err_str) this.kag.error(err_str);
          ...
          this.master_tag[tag.name].start(
            $.extend(!0, $.cloneObject(this.master_tag[tag.name].pm), tag.pm)//Note:合并两个对象，参数以后者为准
          );
        }
...
tyrano.plugin.kag.tag.text = {
  ...
  showMessage: function (message_str, is_vertical) {
    ...
    if ("" == j_msg_inner.html()) {
      ...
      console.log('called!', j_msg_inner);
      this.kag.tmp.last_char_info = null;
    }
    const j_msg_inner_simul = this.kag.getMessageSimulLayer(); 
    if ("" == j_msg_inner_simul.html()) {
      this.kag.setNewParagraph(j_msg_inner_simul);
    }
    this.kag.stat.vchat.is_active && j_msg_inner.show();
    this.kag.checkMessage(j_msg_inner);
    this.kag.checkMessage(j_msg_inner_simul);
    const j_span = this.kag.getMessageCurrentSpan(), 
      j_span_simul = this.kag.getMessageSimulSpan(),
      tmp = this.kag.tmp;
    ...
    this.kag.stat.dual_message==1 && this.setCurrentSpanStyle(j_span_simul, chara_name);
    ...
    const message_html = this.buildMessageHTML(
      ...
      j_message_span = (1==this.kag.stat.block) 
        ? $(`<span class="${this.kag.stat.full_style}">${message_html}</span>`)
        : $(`<span>${message_html}</span>`);
    if (2 == this.kag.stat.block) {
      this.kag.stat.block = 0}
    j_message_span.appendTo(j_span);
    const j_message_span_simul = j_message_span.clone();
    j_span.get(0) && j_span.get(0).scrollIntoView({ behavior: 'smooth', block: 'end' });
    this.kag.stat.fuki.active && this.adjustFukiSize(j_msg_inner, chara_name);
    this.addChars(j_message_span, j_msg_inner, is_vertical,false);
    if(this.kag.stat.dual_message == 1){ 
      j_message_span_simul.appendTo(j_span_simul);
      j_span_simul.get(0) && j_span_simul.get(0).scrollIntoView({ behavior: 'smooth', block: 'end' });
      this.addChars(j_message_span_simul, j_msg_inner_simul, is_vertical,true);
    }
    ...
    for (let i = 0; i < message_str.length; i++) {
      ...
      if (should_check_word_break && undefined === c)
        ...
        if (" " == c)
          ...
          if (1 == this.kag.stat.is_style) {
            c = `<span class="${this.kag.stat.style}">${c}</span>`;
          } else 2 == this.kag.stat.is_style && (this.kag.stat.is_style = 0);
  ...
  addChars: function (j_message_span, j_msg_inner, is_vertical,discard) {
    let ch_speed = 30;
    ...
    if (
      ...
      (this.kag.stat.is_nowait && this.kag.stat.dual_message!=1) || 
      ch_speed <= 3
    ) {
      ...
      this.addOneChar(0, j_char_span_children, j_message_span, j_msg_inner,discard);
    }
  },
...
tyrano.plugin.kag.tag.r = {
  ...
  start: function () {
    this.kag
      ...
      .find(".current_span")
      .append("<br>");
    if(this.kag.stat.dual_message){
      this.kag
      .getMessageSimulLayer()
      .find("p")
      .find(".current_span")
      .append("<br>");  
    }
...
tyrano.plugin.kag.tag.dual = {
  pm: { layer1: "", page1: "fore", layer2: "", page2: "fore" },
  start: function (pm) {
    this.kag.stat.dual_message = 1;
    "" == pm.layer1 && (pm.layer1 = this.kag.stat.current_layer);
    "" == pm.layer2 && (pm.layer2 = this.kag.stat.simultaneous_layer);
    this.kag.stat.current_layer = pm.layer1;
    this.kag.stat.current_page = pm.page1;
    this.kag.stat.simultaneous_layer = pm.layer2;
    this.kag.stat.simultaneous_page = pm.page2;
    this.kag.ftag.nextOrder();
  },
}
...
tyrano.plugin.kag.tag.link = {
  ...
  setEvent: function (j_span, pm) {
    var _target = pm.target,
      ...
      that = TYRANO,
      clicked = !1;
    const once = "false" !== pm.once;//todo: find if this 'once' is a param of the tag,to mark that if the link is clicked
    j_span.on("mousedown", () => !1);//Note:此处采用的是jquery库的on方法：on("event",callback).
    j_span.on("click", (e) => {//where e is the event
...
tyrano.plugin.kag.tag.grouplink = {
  pm: { style:null,link:null,target: null, storage: null, keyfocus: "", once: "true",number:60 },
  padSheet: "丨丿㇏丶㇀乛亅㇁㇂㇇乚㇈㇄㇋㇎㇉㇊亻彳扌忄氵灬宀辶艹礻衤讠钅饣疒犭刂纟丩丬亠冫冖勹匚卩厶夂巛彡攵殳癶罒耂龴",
  start: function (pm) {
    var that = this;
    console.log(pm.target);
    var groups=JSON.parse(pm.link);
    var targets = JSON.parse(pm.target);
    j_span = this.kag.setMessageCurrentSpan();
    const font = this.kag.stat.font;
    j_span.setStyleMap({
      color: font.color,
    ...
    const letter_spacing =
      (this.kag.stat.message_config || {})["letter_spacing"] ||
      this.kag.tag.text.default_message_config["letter_spacing"] ||
      this.kag.config.defaultPitch,
      line_spacing =
        (this.kag.stat.message_config || {})["line_spacing"] ||
        this.kag.tag.text.default_message_config["line_spacing"] ||
        this.kag.config.defaultLineSpacing,
      line_height = parseInt(font.size) + parseInt(line_spacing);
    j_span.setStyleMap({
      "letter-spacing": `${letter_spacing}px`,
    ...
    for (let i = 0; i < groups.length;i++){
      ...
      for(let j=0;j<linkgroup.length;j++){
        ...
        //   j_char_span = $(`<span class="char">${link[k]}</span>`);
        //   j_char_span.appendTo(j_message_span);
        // }
        that.kag.stat.display_link = !0;
        that.kag.setElmCursor(j_message_span, "pointer");
        that.kag.makeFocusable(j_message_span, pm.keyfocus);
        !(function (target) {
          that.kag.event.addEventElement({ tag: "link", j_target: j_message_span, pm: {target:target} });
          that.setEvent(j_message_span, { target: target, storage:null});
        })(target);
        j_message_span.appendTo(j_span);        
      }
    }
    ...
    for(let i=j_span.children().length;i<pm.number;i++){
      ...
      j_padding_span = $(`<span>${pad}</span>`);
      j_padding_span.addClass(pm.style);
      j_padding_span.css({opacity:`${opacity}`});
      j_padding_span.appendTo(j_span);
    }
    this.kag.setMessageCurrentSpan();
    this.kag.ftag.nextOrder();
  },
  ...
  weightedRandom: function (min, max, weightFn) {
    let value;
    do {
      value = Math.random() * (max - min) + min;
    } while (Math.random() > weightFn(value));
    return value;
  },
  weightFn: function (x) {
...
tyrano.plugin.kag.tag.font = {
  ...
  start: function (pm) {
    this.kag.setMessageCurrentSpan();
    if (this.kag.stat.dual_message == 1)this.kag.setMessageSimulSpan();
...
tyrano.plugin.kag.tag.endmark = {
  ...
  start: function (pm) {
    pm.text;
    1 == this.kag.stat.mark && (this.kag.stat.mark = 2);//？？？好神秘的写法
    this.kag.ftag.nextOrder();
  },
};
...
tyrano.plugin.kag.tag.style = {
  ...
  start: function (pm) {
    pm.text;
    this.kag.stat.is_style = 1;
    var style_name = pm.name;
    this.kag.stat.style = style_name;
    this.kag.ftag.nextOrder();
  },
};

tyrano.plugin.kag.tag.endstyle = {
  vital: [],
  pm: {},
  start: function (pm) {
    pm.text;
    1 == this.kag.stat.is_style && (this.kag.stat.is_style = 2);
    this.kag.ftag.nextOrder();
  },
};

tyrano.plugin.kag.tag.block = {
  vital: [],
  pm: {},
  start: function (pm) {
    pm.text;
    this.kag.stat.block = 1;
    var style_name = pm.name;
    this.kag.stat.full_style = style_name;
    this.kag.ftag.nextOrder();
  },
};

tyrano.plugin.kag.tag.endblock = {
  vital: [],
  pm: {},
  start: function (pm) {
    pm.text;
    1 == this.kag.stat.block && (this.kag.stat.block = 2);
    this.kag.ftag.nextOrder();
  },
};
  ...
  setEvent: function (j_button, pm) {
    ...
    const is_fix_button = "true" === pm.fix,
      ...
      is_save_button =
        ...
        "quicksave" == pm.role ||
        "sleepgame" == pm.role,
      exists_call_stack = !!that.kag.getStack("call"),
